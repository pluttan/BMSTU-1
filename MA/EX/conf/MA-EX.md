...
1.  Последовательность §d26§ называется **постоянной**, если §d33§.
...
1.  Последовательность §d26§, ограниченная и сверху и снизу, называют **ограниченной**: §d37§, такое, что §d38§. 
...
1. (определение по Коши) Число §d39§ называется **пределом функции** §d48§ при §d49§, если для любого §d43§ существует положительное число §d50§ такое, что для любого §d51§ выполняется неравенство §d52§. 

    §dd7§

2. (определение по Гейне) Число §d39§ называется **пределом функции** §d48§ при §d49§, если для любой последовательности §d26§ точек из §d53§, для которой §d54§, выполняется равенство §d55§.
...
1. Прямая §d130§ называется **правой наклонной асимптотой** графика функции §d104§, если эту функцию можно представить в виде §d131§  при §d132§, где §d133§ и §d74§ - бесконечно малая функция.

2. Прямая §d130§ называется **левой наклонной асимптотой** графика функции §d104§, если эту функцию можно представить в виде §d131§  при §d134§, где §d133§ и §d74§ - бесконечно малая функция.
...
5. Если §d15§ — точка разрыва функции первого рода и односторонние пределы функции в этой точке равны между собой, но не равны значению функции в этой точке или §d48§ не определена в этой точке, то такой разрыв называют **устранимым**, а точку §d15§ - **точкой устранимого разрыва первого рода**.
    ...
    то функция §d48§ по определению будет непрерывной в точке x = 0.
...
По условию функция §d104§ дифференцируема в точке §d288§ существует конечный §d289§

Функция §d104§ дифференцируема в точке §d288§ Функция §d104§ непрерывна в точке §d290§.
...
Дифференциал функции §d280§ не зависит от того, является ли §d291§ - независимой переменной, или функцией от другой независимой переменной.

<!-- ?\vspace*{20pt} -->
*Доказательство*

1. Пусть §d292§ где §d291§ - независимая переменная.
    Тогда 

    §dd132§

2. Пусть §d292§ где §d274§ - некоторая функция, имеющая производную. Тогда
...
По условию §d104§ дифференцируема в точке §d288§ в точке §d239§ существует конечная производная §d293§
...
Таким образом, для §d313§ выполнены все условия теоремы Ролля §d145§ существует точка §d249§, для которой §d314§
...
Сначала заметим, что §d316§, т.к. если бы §d317§, то §d318§ и функция §d164§, в результате, удовлетворяла бы условию теоремы Ролля, согласно которой §d319§, такая, что §d320§, что противоречит 3 условию теоремы.
...
Доопределим функции §d48§ и §d164§ в точке §d328§ полагая §d329§.
Тогда §d48§ и §d164§ определены и непрерывны в §d330§. Рассмотрим отрезок §d331§ где §d332§.

По условию §d333§ в §d334§.

Тогда функции §d48§ и §d164§ удовлетворяют условию теоремы Коши, а значит, §d335§, такая, что §d336§

1. Так как §d337§ то §d338§

2. Так как §d339§, то, если §d56§, то и §d340§
...
1. Сравним рост показательной функции §d343§ и степенной функции §d344§ (§d345§, §d346§):

    Так как при §d132§ функции §d343§ и §d344§ являются бесконечно большими, воспользуемся правилом Лопиталя-Бернулли §d27§ раз:

    §dd156§

    Таким образом, показательная функция §d343§ растет быстрее степенной функции §d344§.

2. Сравним рост логарифмической функции §d347§ и степенной функции §d348§:

    Так как при §d132§ функции §d349§ и §d344§ являются бесконечно большими, воспользуемся правилом Лопиталя-Бернулли:

    §dd157§

    Таким образом, степенная функция §d344§ растет быстрее логарифмической функции §d349§.
...
Пусть функция §d48§ определена в окрестности §d350§ точки §d15§ и имеет в этой окрестности производные всех порядков до §d351§-го включительно. Тогда для любого §d352§ справедливо равенство:

> §dd158§
> 
> §dd159§

Пусть §d352§, и пусть для определенности §d332§. Рассмотрим на отрезке §d353§ две функции
...
В последней сумме введем новый индекс суммирования §d354§. Тогда
...
Далее, §d355§
, и непосредственно видно, что производная §d356§ на
интервале §d334§ отлична от нуля. К паре функций §d357§ и §d358§ на отрезке §d353§ применим теорему Коши. Имеем
...
Из последнего равенства следует утверждение теоремы при §d332§. При §d360§ рассуждения аналогичны; если §d239§, то утверждение теоремы очевидно. 
...
<!-- ?\newpage -->
### 38. *Формула Маклорена для функции §d363§ с остаточным членом в форме Лагранжа*
...
Найдем производные функции §d243§ до §d364§-го порядка:
...
Найдем производные функции §d246§ до §d365§-го порядка:
...
<!-- ?\newpage -->
### 41. *Формула Маклорена для функции §d366§ с остаточным членом в форме Лагранжа*

Найдем производные функции §d367§ до §d368§-го порядка:
...
<!-- ?\newpage -->
### 42. *Формула Маклорена для функции §d369§ с остаточным членом в форме Лагранжа*

Найдем производные функции §d369§ до §d27§-го порядка:
§dd213§
§dd214§
§dd215§
§dd216§
...
Пусть функция §d48§ дифференцируема на интервале §d93§. Для того, чтобы эта функция была неубывающей на интервале §d93§, необходимо и достаточно, чтобы производная §d370§ была неотрицательна §d371§
...
§d93§. Тогда в точке §d372§, в которой функция §d48§ дифференцируема, имеем
...
По условию во всех точках интервала §d376§, в которых §d48§ дифференцируема, выполняется неравенство §d377§. Пусть §d309§ и §d378§ — произвольные точки этого промежутка. Тогда функция §d104§ удовлетворяет теореме Лагранжа, §d379§, такая, что  §d380§

Т.к. §d375§, и §d112§, то §d381§
...
Пусть функция §d48§ дифференцируема на интервале §d93§. Для того, чтобы эта функция была невозрастающей на интервале §d93§, необходимо и достаточно, чтобы производная §d382§
...
По условию во всех точках интервала §d376§, в которых §d48§ дифференцируема, выполняется неравенство §d386§. Пусть §d309§ и §d378§ — произвольные точки этого промежутка. Тогда функция §d104§ удовлетворяет теореме Лагранжа, §d387§, такая, что  §d380§

Т.к. §d385§, и §d112§, то §d388§
...
Пусть функция §d104§ непрерывна в §d237§ и дифференцируема в §d240§ Тогда, если §d370§ меняет знак с минуса на плюс при переходе через точку §d15§, то в этой точке функция
§d48§ имеет строгий локальный минимум, а если §d370§ меняет знак с плюса на минус при
переходе через §d15§, то функция §d48§ имеет в этой точке строгий локальный максимум.
Если же §d370§ сохраняет знак в проколотой окрестности точки §d15§, то экстремума в этой
точке нет.

<!-- ?\vspace*{20pt} -->
*Доказательство*

Рассмотрим первое утверждение теоремы. Если §d389§ при
всех §d390§, то на полуинтервале §d391§ функция §d48§ убывает, и для
любого §d390§ имеем §d392§. На полуинтервале §d393§ функция §d48§ возрастает,
и §d394§ для всех §d395§. Мы видим, что §d15§ и в самом деле есть
...
на интервале §d396§ в зависимости от знака производной §d370§; экстремума в
...
Если точка §d239§ - стационарная точка функции §d104§, а функция §d104§ дважды дифференцируема в §d239§ и §d397§, тогда точка §d398§ - точка локального минимума (максимума).

<!-- ?\vspace*{20pt} -->
*Доказательство*

По условию §d399§ функция §d99§ является возрастающей в §d350§

По условию §d400§ - стационарная точка функции §d401§
Таким образом,
§dd221§
...
дифференцируема на интервале §d93§, причем в каждой точке §d372§ выполняется
неравенство §d403§. Тогда функция §d48§ выпукла вниз на указанном интервале.
Если же во всех точках интервала §d93§ вторая производная §d404§ отрицательна, то
...
точке §d405§, §d406§. Уравнение такой касательной имеет вид §d407§. Пусть для определенности §d408§. Тогда разность ординат точки касательной §d409§ и точки графика §d410§ равна §d411§ По теореме Лагранжа
§d412§ Поэтому §d413§
Применим еще раз теорему Лагранжа: §d414§ Здесь §d415§, поэтому §d416§, и точка касательной лежит ниже соответствующей точки графика функции. Аналогично можно доказать это утверждение
и в случае §d417§. Таким образом, точки касательной лежат ниже соответствующих
...
Предположим, §d419§, и пусть для определенности §d420§. Тогда
в силу непрерывности §d421§ в точке §d15§ существует окрестность §d237§
этой точки такая, что §d422§ во всех точках этой окрестности по теореме о сохранении знака непрерывной функции в
окрестности точки. Тогда на обоих
интервалах §d423§ и §d424§ функция §d48§ выпукла вниз, что противоречит
наличию перегиба в точке §d15§. Поэтому предположение §d419§ неверное, §d425§. 
...
Пусть для определенности вторая производная §d421§ положительна при §d390§ и отрицательна при §d395§. Тогда на §d426§ функция
§d48§ выпукла вниз, а на §d424§ выпукла вверх, т.е. при переходе через точку §d15§ направление выпуклости меняется на противоположное. Отсюда следует, что §d15§ - точка
